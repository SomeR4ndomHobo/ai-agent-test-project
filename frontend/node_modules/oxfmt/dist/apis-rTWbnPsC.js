...
async function loadPrettier() {
	return loadCached("prettier", async () => {
		const prettier = await import("./prettier-DQnDZejz.js");
...
async function setupTailwindPlugin(options) {
	CACHES.tailwindPlugin ??= await loadCached("tailwindPlugin", () => import("./dist-Cfco7VuN.js"));
	options.plugins ??= [];
...
async function sortTailwindClasses({ classes, options }) {
	CACHES.tailwindSorter ??= await loadCached("tailwindSorter", () => import("./sorter-DMQ76dSk.js"));
...
async function setupSveltePlugin(options) {
	CACHES.sveltePlugin ??= await loadCached("sveltePlugin", async () => await import("./plugin-ftCrlgi_.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1)));
	options.plugins ??= [];
